...
bot.onText(/\/farrant/, msg => {
  Farrant.farrant(msg, bot, config, MongoClient);
});

bot.onText(/\/add_farrant/, msg => {
  Farrant.addFarrant(msg, bot, config, MongoClient);
});

bot.onText(/\/delete_farrant/, msg => {
  Farrant.deleteFarrant(msg, bot, config, MongoClient);
});

bot.onText(/\/delete_all_farrants/, msg => {
  Farrant.deleteAllFarrants(msg, bot, config, MongoClient);
});
...
bot.onText(/\/add_card/, msg => {
  ...
  if (msg.reply_to_message.photo) {
    ...
    MongoClient.connect(
      ...
      function(err, db) {
        if (err) {
          bot.sendMessage(chatId, "No se ha podido añadir.");
          throw err;
        }
